 ...
 * Created by Timon Paﬂlick, October 21, 2018.
...
//You can tell with "constexpr" in front of a variable declaration that you know also the contents of the array before the program runs and they won't change.
template <typename T, size_t N>
struct StackArray {
  ...
  //You can access the elements of the array before the program runs if it is declared "constexpr" and when the program runs just like with C arrays.
  //If the index is bigger than the array size, the program will crash.
  constexpr T& operator [] (const size_t index) {
    return (index < N) ? c_array[index] : constexprStackArrayIndexOutOfBounds();
  }
  ...
  private:
  //just a helper function to improve the thrown compiler error when accessing an element out of bounds before the program runs
  static inline const T& constexprStackArrayIndexOutOfBounds() {
    abort();
...
class HeapArray {
  ...
  private:
    //A HeapArray is allocated heap space.
    T* begin;
    size_t size;
...
class GrowingArray {
  ...
  private:
    //A GrowingArray is allocated heap space.
  ...
  public:
    //For the sake of simplicity, there is just a default constructor which creates an empty GrowingArray.
    ...
    //You have to copy the GrowingArray explicitly. That prevents you from accidentally passing it by value.
    ...
    HeapArray<T> toHeapArray() {
      HeapArray<T> result{size};
      for (size_t i{0}; i != size; ++i) {
        result[i] = begin[i];
      }
      return result;
